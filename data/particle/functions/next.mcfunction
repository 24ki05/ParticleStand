...
execute as @p unless predicate nsk:sneak as @e[tag=particle,limit=1,sort=nearest] if data storage ps list[0].particle[1] run function particle:next_particle
execute as @p if predicate nsk:sneak as @e[tag=particle,limit=1,sort=nearest] run function particle:next_group
